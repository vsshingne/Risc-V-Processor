...
(
    ...
    begin
        ...
        begin
            // BEQ
            if (!branch_not_equal && !branch_less_than && zero)
                pc_next = pc + immediate;

            // BNE
            else if (branch_not_equal && !branch_less_than && !zero)
                pc_next = pc + immediate;

            // BLT
            else if (branch_less_than && lt_signed)
                pc_next = pc + immediate;

            // BGE
            else if (branch_greater_equal && !lt_signed)
                pc_next = pc + immediate;

            // BLTU
            else if (branch_less_than_unsigned && lt_unsigned)
                pc_next = pc + immediate;

            // BGEU
            else if (branch_greater_equal_unsigned && !lt_unsigned)
                pc_next = pc + immediate;
